# ===============================================================================
# DYNAMIC MODEL - PRETRAINED - TUNNEL 2
# ===============================================================================

set.seed(123)

# pretraining
rf_model_DC <- ranger(DC ~ ., data = df_merge1 %>% drop_na() %>% 
                        select(-Chainage, -DS, -DG),
                      num.trees = 200,
                      # mtry = p,
                      min.node.size = 1,
                      splitrule = 'extratrees') 
rf_model_DS <- ranger(DS ~ ., data = df_merge1 %>% drop_na() %>% 
                        select(-Chainage, -DC, -DG),
                      num.trees = 200,
                      # mtry = p,
                      min.node.size = 1,
                      splitrule = 'extratrees') 
rf_model_DG <- ranger(DG ~ ., data = df_merge1 %>% drop_na() %>% 
...
write_csv(df_result, "../output/df_result_tun2_pt.csv")
write_csv(df_error, "../output/df_error_tun2_pt.csv")
...
for (i in 1:nrow(df_result)) {
  ...
  ggsave(paste0("../output/fig_tun2_pt/fig-", i, ".png"), plot = combined_plot, 
