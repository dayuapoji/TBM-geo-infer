# ===============================================================================
# DYNAMIC MODEL - INCREMENTAL LEARNING - TUNNEL 2
# ===============================================================================
...
for (i in  1:(nrow(df_merge2)-1)) {
  ...
  if (!any(is.na(df_merge2[i, c('DC', 'DS', 'DG')]))) {
    ...
    # train df
    df_train_i <- df_merge2[1:i,] %>% drop_na()
...
write_csv(df_result, "../output/df_result_tun2_inc.csv")
write_csv(df_error, "../output/df_error_tun2_inc.csv")
...
for (i in 1:nrow(df_result)) {
  ...
  ggsave(paste0("../output/fig_tun2_inc/fig-", i, ".png"), plot = combined_plot, 
