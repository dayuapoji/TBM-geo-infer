# ===============================================================================
# DYNAMIC MODEL - INCREMENTAL LEARNING - TUNNEL 1
# ===============================================================================
...
for (i in  1:(nrow(df_merge1)-1)) {
  
  # Check if there are soil labels
  if (!any(is.na(df_merge1[i, c('DC', 'DS', 'DG')]))) {
    
    # train df
    df_train_i <- df_merge1[1:i,] %>% drop_na()
...
write_csv(df_result, "../output/df_result_tun1_inc.csv")
write_csv(df_error, "../output/df_error_tun1_inc.csv")
...
for (i in 1:nrow(df_result)) {
  ...
  ggsave(paste0("../output/fig_tun1_inc/fig-", i, ".png"), plot = combined_plot, 
